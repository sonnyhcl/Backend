...
//		baseVariableCollectionResource.createOrUpdateExecutionVariable2(pid, body);
		...
		result = baseVariableCollectionResource.createExecutionVariableToCacheOrEngine(processInstanceId, true, RestResponseFactory.VARIABLE_PROCESS, request, response , true);
		return new ResponseEntity<List<RestVariable>>(result ,HttpStatus.OK);
	}
	...
	public ResponseEntity<RestVariable> updateVariableByNameToCacheAndEngine(@PathVariable String processInstanceId, @PathVariable String variableName,
			HttpServletRequest request) throws JsonProcessingException {
